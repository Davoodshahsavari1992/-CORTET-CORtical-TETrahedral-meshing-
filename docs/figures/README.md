...
| `cortet_bad_elements.png` | Elements over the limit (`q > 0.6`) on the same week-22 surface: TetGen (`pq1.2/20Y`, as in `run/04`), the CGAL fill alone, and CORTET | `run/04` and `run/03` on the week-22 surface |
